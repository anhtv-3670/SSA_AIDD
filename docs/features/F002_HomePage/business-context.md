...
1. Điều hướng tới `/home` (trực tiếp hoặc qua ứng dụng — sau đăng nhập hiện vẫn về `/`).
2. Thấy lời chào kèm định danh tài khoản (`email`, hoặc `phone`, hoặc `id` nếu thiếu email/phone).
3. Bấm "Đăng xuất" khi muốn kết thúc phiên → quay về `/login`.
4. Nếu phiên hết hạn / chưa đăng nhập mà mở `/home` → được đưa về `/login`.
